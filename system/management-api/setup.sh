...
# API URL is configured here for frontend build

# API URL - defaults to empty string (same origin) if not specified
...
generate_backend_env() {
    ...
    local password="${AUTH_PASSWORD:-changeme123}"
    ...
    if [ "$username" = "admin" ] && [ "$password" = "changeme123" ]; then
        ...
        log_warn "Please change credentials in $env_file after installation"
    fi
...
build_frontend_in_source() {
    ...
    # Generate frontend .env.local (no credentials needed anymore)
...
install() {
    ...
    log_info "  Password: changeme123"
    log_info ""
    log_info "To change credentials:"
    log_info "  1. Edit: sudo nano $CONFIG_DIR/.env"
    log_info "  2. Update AUTH_USERNAME and AUTH_PASSWORD"
    log_info "  3. Restart: sudo systemctl restart $SERVICE_NAME"
    log_info ""
...
build() {
    ...
    # Note: Credentials will use defaults from .env.example (admin/changeme123)
    # Users should change credentials in /etc/luigi/system/management-api/.env after installation
    log_info "Building with default credentials (change in /etc/luigi/system/management-api/.env after installation)"
