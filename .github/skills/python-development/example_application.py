#!/usr/bin/env python3
"""
Example Motion Detection Application - Refactored Version

This is a complete, production-ready example demonstrating best practices
for Python hardware development on Raspberry Pi Zero W.
...
- Structured logging
- Graceful shutdown
- Mock GPIO support for development
...
try:
    ...
    class MockGPIO:
        ...
        def setup(cls, pin, direction, **kwargs):
            ...
            print(f"[MOCK] Pin {pin} setup as {direction}")
        ...
        def add_event_detect(cls, pin, edge, callback=None, bouncetime=None):
            print(f"[MOCK] Event detect on pin {pin}, edge {edge}")
...
class MotionDetectionApp:
    ...
    def run(self):
        ...
        # Main loop - just sleep and let events handle everything
        try:
            while self.running:
...
def main():
    ...
    setup_logging()
    logging.info("=" * 60)
    logging.info("Motion Detection Application Starting")
    logging.info("=" * 60)
